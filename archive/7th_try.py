...
def main():
    ...
    model = ensemble.StackingRegressor(
        ...
        final_estimator=svm.SVR(C=30, gamma='scale', epsilon=0.1, kernel='linear'), 
    )
    ...
    print(f"Final Regression: Cross-validated R^2: {score.mean():.4f} (+/- {score.std() * 2:.4f} std)")
    ...
    # print(f"Final Regression: Out-of-sample R^2 Score: {out_of_sample_r2:.4f}")
...
def fill_missing_values(X_train, strategy: str, X_test=None):
    ...
    if strategy in ['mean', 'median', 'most_frequent']:
        ...
        imputer = impute.IterativeImputer(max_iter=10, random_state=RANDOM_STATE, n_nearest_features = 20, verbose = 10)
    else:
