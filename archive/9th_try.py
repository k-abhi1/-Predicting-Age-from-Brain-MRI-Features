...
def main():
    ...
    model = ensemble.StackingRegressor(
        ...
        # final_estimator=linear_model.LassoCV(cv=5, n_jobs=-1, random_state=RANDOM_STATE), # 0.6374
        final_estimator=svm.SVR(C=30, kernel='linear', gamma='scale', epsilon=0.1), # 0.6377
        # final_estimator=linear_model.RidgeCV(),   # 0.6373
        # n_jobs=-1,
        # verbose=10,
    ) 
...
def select_features(X_train, y_train, X_test, method: list[str], variance_threshold: float = 0.0, k: int = 200, alpha: float = 0.01, corr_threshold: float = 0.97, target_corr_threshold: float = 0.09):
    """
