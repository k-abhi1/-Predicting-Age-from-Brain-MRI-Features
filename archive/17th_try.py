...
def main():
    ...
    plot_histograms(pd.DataFrame(X_train_selected))
    indices = [1,29,78,89,91,95,161,163,165,166,167,169,172,173]
    pt = PowerTransformer(method='yeo-johnson')
    for index in indices:
    # Transform the column and update it in the numpy array
        transformed_column = pt.fit_transform(X_train_selected[:, index].reshape(-1, 1))
        X_train_selected[:, index] = transformed_column.flatten()
    ...
    # Best SVR Model: {'C': 50.9413801481638, 'epsilon': 0.0001, 'gamma': 'auto', 'kernel': 'rbf'}
    # Found using above GridSearchCV
    svr = svm.SVR(C=50.9413801481638, epsilon=0.0001, gamma='auto', kernel='rbf')  # 0.6356
    ...
    #Best KNN Model: {'algorithm': 'kd_tree', 'leaf_size': 1, 'metric': 'manhattan', 'n_jobs': -1, 'n_neighbors': 3, 'p': 1, 'weights': 'distance'} (R^2: 0.5403)
    # Found using above GridSearchCV
    knn = neighbors.KNeighborsRegressor(n_neighbors=3, weights='distance', p=2, metric='manhattan', leaf_size=50, algorithm='auto') # 0.5326
...
def plot_histograms(data):
    indices = [1, 29, 78, 89, 91, 95, 161, 163, 165, 166, 167, 169, 172, 173]
    chosen_features = data.columns[indices]

    # Define number of features per page and calculate pages needed
    features_per_page = 2
    num_pages = (len(chosen_features) + features_per_page - 1) // features_per_page

    for page in range(num_pages):
        fig, axes = plt.subplots(features_per_page, 4, figsize=(10, 10))
        fig.suptitle(f"Page {page + 1}: Original and Yeo-Johnson Transformations with Q-Q Plots", fontsize=16)

        # Get the start and end indices for this page
        start_idx = page * features_per_page
        end_idx = min(start_idx + features_per_page, len(chosen_features))
        
        # Plot each selected feature's original and transformed distributions
        for idx, feature_name in enumerate(chosen_features[start_idx:end_idx]):
            original_data = data[feature_name]
            
            # Original distribution plot
            sns.histplot(original_data, kde=True, ax=axes[idx, 0])
            axes[idx, 0].set_title(f"{feature_name} - Original Distribution")
            
            # Q-Q plot for the original data
            stats.probplot(original_data, dist="norm", plot=axes[idx, 1])
            axes[idx, 1].set_title(f"{feature_name} - Original Q-Q Plot")
            
            # Yeo-Johnson transformation
            pt = PowerTransformer(method='yeo-johnson')
            yeo_johnson_transformed = pt.fit_transform(original_data.values.reshape(-1, 1)).flatten()
            
            # Yeo-Johnson transformed distribution plot
            sns.histplot(yeo_johnson_transformed, kde=True, ax=axes[idx, 2])
            axes[idx, 2].set_title(f"{feature_name} - Yeo-Johnson Transformation")
            
            # Q-Q plot for the Yeo-Johnson-transformed data
            stats.probplot(yeo_johnson_transformed, dist="norm", plot=axes[idx, 3])
            axes[idx, 3].set_title(f"{feature_name} - Yeo-Johnson Q-Q Plot")
